...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 qNet
      real*8 mob,dmobdu(2),dmobdphi,dmobdomg
      real*8 dcbardu(2),dcbardphi,dcbardomg
      real*8 SpTanMod(3,3,3,3),Spuomgmod(3,3),Spuphimod(3,3),Spomgumod(3,3,3)
	  real*8 amatcu(2,4),Spphiumod(3,3,3)
      !
      ...
      real*8 dsh(nNode,2),detMapJC
      real*8 dshC(nNode,2),xi(nInt,2)
      real*8 dshxi(nNode,2),dsh0(nNode,2),dshC0(nNode,2),detMapJ0C
      real*8 w(nInt),ds,flux

      ! residuals and tangents
      real*8 Ru(2*nNode,1),Romg(nNode,1),Rphi(nNode,1)
      real*8 Kuu(2*nNode,2*nNode),Komgomg(nNode,nNode)
      ...
      real*8 Komgu(nNode,2*nNode),Kuomg(2*nNode,nNode)
...
!         enddo
     ...
     +        (
     +        ( dshC(i,1)*dshC(j,1) + dshC(i,2)*dshC(j,2) )
     ...
     +                 + domgdx(k)*mob*iden(i,l)
         ...
         AmatCU(1,3) = SpomgUMod(1,1,2)
         AmatCU(1,4) = SpomgUMod(1,2,2)
         AmatCU(2,1) = SpomgUMod(2,1,1)
         AmatCU(2,2) = SpomgUMod(2,2,1)
         AmatCU(2,3) = SpomgUMod(2,1,2)
         AmatCU(2,4) = SpomgUMod(2,2,2)
         !
     ...
     +                 + dphidx(k)*iden(i,l)
         ...
         AmatCU(1,3) = SpphiUMod(1,1,2)
         AmatCU(1,4) = SpphiUMod(1,2,2)
         AmatCU(2,1) = SpphiUMod(2,1,1)
         AmatCU(2,2) = SpphiUMod(2,2,1)
         AmatCU(2,3) = SpphiUMod(2,1,2)
         AmatCU(2,4) = SpphiUMod(2,2,2)
         !
      ...
      elseif(nDim.eq.3) then
         ...
         do i=1,nNode
            do j=1,nNode
               ...
               ! temperature
               !
               amatrx(A11+4,B11+4) = Komgomg(i,j)
               !
               ! displacement - chemical potential
               !
               ...
               ! chemical potential - temperature
               !
               amatrx(A11+3,B11+4) = Kphiomg(i,j)
               !
               ! temperature - displacement
               !
               amatrx(A11+4,B11)   = Komgu(i,B12)
               amatrx(A11+4,B11+1) = Komgu(i,B12+1)
               amatrx(A11+4,B11+2) = Komgu(i,B12+2)
               !
               ! temperature - chemical potential
               !
               amatrx(A11+4,B11+3) = Komgphi(i,j)
               !
            enddo
         enddo
         !
