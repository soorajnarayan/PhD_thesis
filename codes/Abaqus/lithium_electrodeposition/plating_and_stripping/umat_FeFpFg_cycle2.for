...
c
     ...
     +     T_tau,plasticwork)
      ...
      elseif(ntens.eq.4) then
        ...
        !
        dE = 1.d-6
        dE = max(nuP_tau*1.d0*1.d-4,1.d-9)
        perM = zero
...
c     ! To model a flux cycling
c     !
c      if(step_time.lt.4000.d0) then
c       cDot = 2*cDot
c      elseif(mod(floor(step_time/2000.d0),2).eq.0)then
c       cDot = -2*cDot
c      else 
c       cDot = 2*cDot
c      endif  
      ...
      !-extra
      ! This sinppet helps to model a flux cycling
      !
      if(step_time.lt.2160.d0) then
       cDot = cDot
      elseif(mod(floor(step_time/2160.d0),2).eq.0)then
       cDot = cDot
      else 
       cDot = -cDot
      endif  
