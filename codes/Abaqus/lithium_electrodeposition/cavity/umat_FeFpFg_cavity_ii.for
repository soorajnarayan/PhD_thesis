...
!      if( ( (coords(1).ge.50.d0).and.(coords(1).le.75.d0) ) 
!     &     .or.( (coords(1).ge.325.d0).and.(coords(1).le.350.d0) ) )then
!          cDot = cDot/10
!      endif    
      
c      if ( (coords(1).ge.0.d0).and.(coords(1).le.5.d0) ) then
c          cDot = cDot/10
c      endif    

c      if ( (coords(1).ge.10.d0).and.(coords(1).le.30.d0) ) then
c          cDot = cDot/10
c      endif       

c      if ( (coords(1).ge.35.d0).and.(coords(1).le.40.d0) ) then
c          cDot = cDot/10
c      endif       
      ...
      x_pt = coords(1)
      y_pt = coords(2)
      ! The below snippet does the 'closest-point-on-the-interface' calculation and 
      ! the corresponding growth direction evaluations for the specific cavity geometry in play.
      if(cmname(1:5).eq.'IP')then
      !-    if(step_time.eq.zero) then
          !write(*,*),'Here'
          n_vec = zero
      ! Signs need to be handled with care
        if(x_pt.ge.0d0)then  
          xC1 = 0.
          yC1 = 2.
          R1 = 0.5
          ...
          mSlope = -6.3147d0
          cInt   = 5.1967
              if(x_pt.gt.1.25d0)then
              n_vec(2) = 1.d0
              xSurf = x_pt
              elseif((atan((y_pt-yC1)/(x_pt-xC1)).le.pi/2.d0).and.
     +           (atan((y_pt-yC1)/(x_pt-xC1)).ge.pi/20.d0))then
              theta_subtended = atan((y_pt-yC1)/(x_pt-xC1))
              n_vec(2) = sin(theta_subtended)
              n_vec(1) = cos(theta_subtended)
              xSurf = xC1 + R1*(cos(theta_subtended))
              elseif((atan((x_pt-xC2)/(y_pt-yC2)).ge.zero).and.
     +           (atan((x_pt-xC2)/(y_pt-yC2)).le.1.41374d0))then
              theta_subtended = atan((x_pt-xC2)/(y_pt-yC2))
              n_vec(2) = cos(theta_subtended)
              n_vec(1) = sin(theta_subtended)
              xSurf = xC2 - R2*(sin(theta_subtended))
              else
              n_vec(2) = cos(1.41374)
              n_vec(1) = sin(1.41374)
              xSurf = (mSlope*y_pt+x_pt-mSlope*cInt)/(1+mSlope**two)
              endif
        else  
          xC1 = 0.
          yC1 = 2.
     ...
     +           (atan((x_pt-xC2)/(y_pt-yC2)).le.1.41374d0))then
      ...
      ! This next snippet does the mapping of the directions vectors between 
      ! the deformed, intermediate, and reference spaces
      Fmap_t = matmul(Fg_t,Finv_t)
          m_g2 = zero   
          do i=1,3
              do j=1,3
      ...
      ! The following snippet implements the inhomogeneous current distribution.
      ! The distribution was obtained by solving just the Ohm's law problem; 
      ! the variation was fitted to piecewise polynomial functions as below.   
      if(xSurf.le.0.55d0)then
      cDot=cDot*(-34927.d0*xSurf**8.d0+85851.d0*xSurf**7.d0
     + -85274.d0*xSurf**6.d0 +43923.d0*xSurf**5.d0-12520.d0*xSurf**4.d0
     + +1942.9*xSurf**3.d0-150.14d0*xSurf**2.d0 +4.3938d0*xSurf + 
     + +0.98514 ) 
      !write(*,*)'in here',xSurf
      elseif((xSurf.gt.0.55d0).and.(xSurf.le.1.1d0))then
!      cDot=cDot*(6537.4*xSurf**8.d0-45647.d0*xSurf**7.d0
!     + +1.3857d5*xSurf**6.d0 -2.388d5*xSurf**5.d0 +2.5547d5*xSurf**4.d0
!     + -1.7369d5*xSurf**3.d0+73280.d0*xSurf**2.d0-17539.d0*xSurf+1823.4)
      cDot=cDot*( 
     + 1.8077d0*xSurf**5.d0 -6.141d0*xSurf**4.d0 +4.6586d0*xSurf**3.d0
     + +5.1028d0*xSurf**2.d0-8.5771d0*xSurf      +3.1933d0)
      !write(*,*)'in here',xSurf
      else
!      cDot=cDot*(
!     + -1.4487d-7*xSurf**6.d0+8.5026d-6*xSurf**5.d0-2.051d-4*xSurf**4.d0
!     ++2.625d-3*xSurf**3.d0-0.019151*xSurf**2.d0+0.07818*xSurf-0.048078) 
      cDot=cDot*(
     + 4.3051d-6*xSurf**5.d0-0.00021255*xSurf**4.d0+0.004117*xSurf**3.d0
     + -0.039602*xSurf**2.d0+0.19506*xSurf-0.11708)
      endif
      cDot = cDot*16.d0/5.d0 ! multipltying factor used to normalize the fit and divide by total current since that comes in through cDot
