...
c      if( ( (coords(1).ge.35.d0).and.(coords(1).le.65.d0) ) 
c     &     .or.( (coords(1).ge.435.d0).and.(coords(1).le.465.d0) ) )then
c          cDot = cDot/10
c      endif    
     ...
     ! The following snippet switches off the deposition after decohesion.
      if(step_time.gt.700.d0) then     
      if( ( (coords(1).ge.18.75d0).and.(coords(1).le.50.d0) ) 
     & .or.( (coords(1).ge.450.d0).and.(coords(1).le.481.25d0) ) )then
          cDot = 0.
       endif   
      endif
