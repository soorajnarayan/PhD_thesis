...
! 
!
! Lallit Anand, January 2019, Implemented in Abaqus 6.17
!
************************************************************************
!     State Variables
!     --------------------------------------------------------------
...
!**********************************************************************
     ...
     + props(nprops),coords(3),drot(3,3),dfgrd0(3,3),dfgrd1(3,3)
      ...
      real*8 Eyoung,poisson,Apre,Qact,T0,mRate,S0,H0,Ssat,ahard
      real*8 Y0,Ysat,nhard
      ...
      ! Parameters
      !
      real*8 zero,one,two,half,three,third,nine,root_three,Rgas
...
c************************ Initialize
     ...
     +     jobName(1:lenJobName)//'.dat'
      ...
      ahard   = props(10)  ! Exponent in hardening relation 
      nhard   = props(11)  ! Exponent in hardening relation      
      eBarLmt = props(12) ! Limit on Delta ebarp      
      ...
      !Write(80,*)'eBarLmt = ',eBarLmt


c     	When umat is called for the first iteration in an 
...
c
      ...
      if(time(2).eq.zero) then
         ...
         Fp_t(1,1) = statev(1)  ! Fp at time t
         Fp_t(2,2) = statev(2)  ! Fp at time t
         Fp_t(3,3) = statev(3)  ! Fp at time t
         Fp_t(2,3) = statev(4)  ! Fp at time t
         Fp_t(3,2) = statev(5)  ! Fp at time t
         Fp_t(1,3) = statev(6)  ! Fp at time t
         Fp_t(3,1) = statev(7)  ! Fp at time t
         Fp_t(1,2) = statev(8)  ! Fp at time t
         Fp_t(2,1) = statev(9)  ! Fp at time t
     ...
     +     F_tau,Fp_t,nuP_t,S_t,Y_t,gBarP_t,eBarP_t,
     +     Fp_tau,nuP_tau,S_tau,Y_tau,gBarP_tau,eBarP_tau,
     +     T_tau,plasticwork)
      ...
      ! Return the volumetric heat generated caused by plasticity
      !
      rpl = plasticwork
      ...
      !endif
        ...
        call integ(props,nprops,dtime,
     +     F_per,
     +     Fp_t,nuP_t,S_t,Y_t,gBarP_t,eBarP_t, 
     +     Fp_tau,nuP_tau,S_tau,Y_tau,gBarP_tau,eBarP_tau,
     +     T_per,plasticwork) 
...
***********************************************************************
     ...
     +     T_tau,plasticwork)
      ...
      real*8 Eyoung,poisson,Apre,Qact,T0,mRate,S0,H0,Ssat,ahard
      real*8 Y0,Ysat,H_t,H_init,nu0,nhard
      real*8 Gshear,Kbulk,Lambda,eBarLmt
      ...
      ! Parameters
      !
      real*8 zero,one,two,three,half,third,four,Pi,two_third,six,root_three
      real*8 Rgas
...
********************************************************************* 
      ...
      !H_init = H0*(one-(S0/Ssat))**ahard
      !write(80,*)'H_init = ',H_init
      ...
      ! Compute the mean normal pressure (this is theactual value
      !  at the end of the increment as well as the trial value)
      !
     ...
     +               nup_tau,xacc,args,nargs,stat)
      ...
      ! Compute the Mandel stress with thermal effects
      !  accounted for at the end of the increment
      !
     ...
     +                 + (Kbulk-(two/three)*Gshear*ratio)*Iden(i,j)*Iden(k,l)
               enddo
            enddo
         enddo
      enddo
     ...
     +        (
     +        Gshear*dtime 
     +        + dSdnu*((nuP_tau/nu0)**mRate) 
     +        + (S_tau*mRate/nuP_tau)*((nuP_tau/nu0)**mRate)
     +        )
         !
         MatTan = zero
         do i=1,3
     ...
     +                    - factor2*Np(i,j)*Np(k,l)
      ...
      ! plastic work dissipated, per unit volume, per unit time
      !
