...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      parameter(nIntS=4) ! number of surface integration points
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     +     (lflags(1).eq.72).or.(lflags(1).eq.73)) then
      ...
      if(jtype.eq.1) then
         ...
         !
         nDim = 3
         call U3D8a(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
     +        PROPS,NPROPS,coords,MCRD,NNODE,Uall,DUall,Vel,Accn,JTYPE,
     +        TIME,DTIME,KSTEP,KINC,JELEM,PARAMS,NDLOAD,JDLTYP,ADLMAG,
     +        PREDEF,NPREDF,LFLAGS,MLVARX,DDLMAG,MDLOAD,PNEWDT,JPROPS,
     +        NJPROP,PERIOD,
     +        nDim,nInt,nIntS)
      ...
      elseif(jtype.eq.2) then
         ...
         !
         nDim = 3
C         call U3D8b(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
...
************************************************************************
      ...
      subroutine U3D8a(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
     +     PROPS,NPROPS,coords,MCRD,NNODE,Uall,DUall,Vel,Accn,JTYPE,
     +     TIME,DTIME,KSTEP,KINC,JELEM,PARAMS,NDLOAD,JDLTYP,ADLMAG,
     +     PREDEF,NPREDF,LFLAGS,MLVARX,DDLMAG,MDLOAD,PNEWDT,JPROPS,
     +     NJPROP,PERIOD,
...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 omgP_tau,omgP_t,domgPdx(3,1),domgPdt
      real*8 omgN_tau,omgN_t,domgNdx(3,1),domgNdt
      real*8 omgH_tau,omgH_t,domgHdx(3,1),domgHdt
      real*8 omgOH_tau,omgOH_t,domgOHdx(3,1),domgOHdt
      real*8 mu_tau,mu_t,dmudx(3,1),dmudt
      real*8 F_tau(3,3),F_t(3,3),detF_tau,detF,detF_t
      real*8 phi_tau,phi_t,dphidx(3,1),dphidt
      ...
      real*8 Ru(3*nNode,1),Rmu(nNode,1),RomgP(nNode,1),RomgN(nNode,1)
      real*8 Rphi(nNode,1),RomgH(nNode,1),RomgOH(nNode,1)
      real*8 Kuu(3*nNode,3*nNode),Kmumu(nNode,nNode)
      real*8 Kphiphi(nNode,nNode),KomgPomgP(nNode,nNode)
      real*8 KomgNomgN(nNode,nNode),KomgHomgH(nNode,nNode)
      real*8 KomgOHomgOH(nNode,nNode)
      real*8 Kuphi(nDim*nNode,nNode),Kphiu(nNode,nDim*nNode)
      real*8 Kmuphi(nNode,nNode),Kphimu(nNode,nNode)
      real*8 Kmuu(nNode,3*nNode),Kumu(3*nNode,nNode)
      real*8 KomgPphi(nNode,nNode),KphiomgP(nNode,nNode)
      real*8 KomgPu(nNode,3*nNode),KuomgP(3*nNode,nNode)
      real*8 KomgNphi(nNode,nNode),KphiomgN(nNode,nNode)
      real*8 KomgNu(nNode,3*nNode),KuomgN(3*nNode,nNode)
      ...
      real*8 key
      ! FEM quantities
      real*8 sh(nNode),detMapJ,dshC(nNode,3),w(nInt)
      real*8 sh0(nNode),detMapJ0,xi(nInt,3)
      ...
      real*8 zeta_t,dsh(nNode,3),detMapJC,zetaLmt
      ...
      real*8 dmobSdmu,Gmat(9,3*nNode),G0mat(9,3*nNode),Amat(9,9)
      real*8 Nvec(1,nNode),ResFac,AmatUC(6,1),TanFac
      real*8 dcPdphi,dcNdphi,dmobPdphi,dmobNdphi
      real*8 SpumuMod(3,3),Spmuumod(3,3,3),SpmuumodFac(3,3),AmatCU(3,9)
...
c         deallocate(globalSdv)
     ...
     +     ((coordsC(3,1)-coordsC(3,7))**two))
      ...
      !
         call mapShape3Da(nNode,dshxi,coords,dsh0,detMapJ0,stat,pnewdt)
         if(stat.eq.0) then
            pnewdt = 0.5
            return
         endif
      ...
      !
       call mapShape3Da(nNode,dshxi,coordsC,dshC0,detMapJ0C,stat,pnewdt)
         if(stat.eq.0) then
            pnewdt = 0.5
            return
         endif
      ...
      do intpt=1,nIntPt
         ...
         ! Map shape functions from local to global reference coordinate system
         !
         call mapShape3Da(nNode,dshxi,coords,dsh,detMapJ,stat,pnewdt)
            if(stat.eq.0) then
               pnewdt = 0.5
               return
            endif
         ...
         ! Map shape functions from local to global current coordinate system
         !
         call mapShape3Da(nNode,dshxi,coordsC,dshC,detMapJC,stat,pnewdt)
            if(stat.eq.0) then
               pnewdt = 0.5
               return
            endif
     ...
     +        SpumuMod,SpmuumodFac)
         ...
         globalSdv(jelem,intPt,4) = key
         globalSdv(jelem,intPt,5) = zeta_tau
         globalSdv(jelem,intPt,6) = cRf_dissoc
         ...
         if(umeror.le.half) then
            pnewdt = min(1.5d0,pnewdt)
         elseif((umeror.gt.half).and.(umeror.le.0.8d0)) then
            pnewdt = min(1.25d0,pnewdt)
         elseif((umeror.gt.0.8d0).and.(umeror.le.1.25d0)) then
            pnewdt = min(0.75d0,pnewdt)
         else
            pnewdt = min(half,pnewdt)
            write(*,*)'Cutback with zeta'
         endif
...
!     +                              *(-one/zeta_tau**two)*dzetadmu 
     ...
     +                 + dMUdX(k,1)*SpmuumodFac(i,l)
         ...
         AmatCU(1,3) = SpmuUMod(1,3,1)
         AmatCU(1,4) = SpmuUMod(1,1,2)
         AmatCU(1,5) = SpmuUMod(1,2,2)
         AmatCU(1,6) = SpmuUMod(1,3,2)
         AmatCU(1,7) = SpmuUMod(1,1,3)
         AmatCU(1,8) = SpmuUMod(1,2,3)
         AmatCU(1,9) = SpmuUMod(1,3,3)
         AmatCU(2,1) = SpmuUMod(2,1,1)
         AmatCU(2,2) = SpmuUMod(2,2,1)
         AmatCU(2,3) = SpmuUMod(2,3,1)
         AmatCU(2,4) = SpmuUMod(2,1,2)
         AmatCU(2,5) = SpmuUMod(2,2,2)
         AmatCU(2,6) = SpmuUMod(2,3,2)
         AmatCU(2,7) = SpmuUMod(2,1,3)
         AmatCU(2,8) = SpmuUMod(2,2,3)
         AmatCU(2,9) = SpmuUMod(2,3,3)
         AmatCU(3,1) = SpmuUMod(3,1,1)
         AmatCU(3,2) = SpmuUMod(3,2,1)
         AmatCU(3,3) = SpmuUMod(3,3,1)
         AmatCU(3,4) = SpmuUMod(3,1,2)
         AmatCU(3,5) = SpmuUMod(3,2,2)
         AmatCU(3,6) = SpmuUMod(3,3,2)
         AmatCU(3,7) = SpmuUMod(3,1,3)
         AmatCU(3,8) = SpmuUMod(3,2,3)
         AmatCU(3,9) = SpmuUMod(3,3,3)
         !
...
!     +        + matmul(matmul(dshC,AmatCU),Gmat)
     +        )
         ...
         AmatUC(3,1) = SpUmuMod(3,3)
         AmatUC(4,1) = SpUmuMod(1,2)
         AmatUC(5,1) = SpUmuMod(2,3)
         AmatUC(6,1) = SpUmuMod(1,3)
         !
     ...
     +     rhs,amatrx)
      ...
      elseif (key.gt.half)then
      call AssembleElement2(nDim,nNode,nDofEl,
     +     Ru,Rmu,RomgP,RomgN,Rphi,RomgH,
     +     Kuu,Kumu,Kmuu,Kmumu,KomgPomgP,KomgNomgN,Kphiphi,KomgHomgH,
     +     KomgPphi,KomgNphi,KphiomgP,KphiomgN,
     +     KmuomgN,KmuomgP,KomgPmu,KomgNmu,
...
****************************************************************************
      ...
      rootMax = 0.999999999999d0 ! corresponds to nearly 100% dry polymer
     ...
     +     rhs,amatrx)
      ...
      if(nDim.eq.3) then
         ...
         do i=1,nNode
            ...
            ! displacement
            !
            rhs(A11,1) = Ru(A12,1)!/1.d3
            rhs(A11+1,1) = Ru(A12+1,1)!/1.d3
            rhs(A11+2,1) = Ru(A12+2,1)!/1.d3
            !
         ...
         do i=1,nNode
            do j=1,nNode
               ...
               ! displacement
               !
               amatrx(A11,B11)     = Kuu(A12,B12)!/1.d3
               amatrx(A11,B11+1)   = Kuu(A12,B12+1)!/1.d3
               amatrx(A11,B11+2)   = Kuu(A12,B12+2)!/1.d3
               amatrx(A11+1,B11)   = Kuu(A12+1,B12)!/1.d3
               amatrx(A11+1,B11+1) = Kuu(A12+1,B12+1)!/1.d3
               amatrx(A11+1,B11+2) = Kuu(A12+1,B12+2)!/1.d3
               amatrx(A11+2,B11)   = Kuu(A12+2,B12)!/1.d3
               amatrx(A11+2,B11+1) = Kuu(A12+2,B12+1)!/1.d3
               amatrx(A11+2,B11+2) = Kuu(A12+2,B12+2)!/1.d3
               !
               ! electrical potential
               !
               ...
               ! chemical potential
               !
               amatrx(A11+7,B11+7) = KomgHomgH(i,j)
               !
               ...
               !
               amatrx(A11+8,B11+8) = 0.d0
               ! displacement - chemical potential
               !
               amatrx(A11,B11+4) = Kumu(A12,j)!/1.d3
               amatrx(A11+1,B11+4) = Kumu(A12+1,j)!/1.d3
               amatrx(A11+2,B11+4) = Kumu(A12+2,j)!/1.d3
               !
               ! chemical potential - displacement
               !
               !
               amatrx(A11+7,B11) = KomgHu(i,B12)
               amatrx(A11+7,B11+1) = KomgHu(i,B12+1)
               amatrx(A11+7,B11+2) = KomgHu(i,B12+2)
               !
               amatrx(A11+4,B11) = Kmuu(i,B12)
               amatrx(A11+4,B11+1) = Kmuu(i,B12+1)
               amatrx(A11+4,B11+2) = Kmuu(i,B12+2)
               !
               !
               amatrx(A11+5,B11) = KomgPu(i,B12)
               amatrx(A11+5,B11+1) = KomgPu(i,B12+1)
               amatrx(A11+5,B11+2) = KomgPu(i,B12+2)
               !
               !
               amatrx(A11+6,B11) = KomgNu(i,B12)
               amatrx(A11+6,B11+1) = KomgNu(i,B12+1)
               amatrx(A11+6,B11+2) = KomgNu(i,B12+2)
               !
               ! echemical potential - electrical potential
               !
               amatrx(A11+7,B11+3) = KomgHphi(i,j)
               !
               amatrx(A11+5,B11+3) = KomgPphi(i,j)
               !
               amatrx(A11+6,B11+3) = KomgNphi(i,j)
               !
               !
               amatrx(A11+3,B11+4) = Kphimu(i,j)
               !
               amatrx(A11+4,B11+3) = Kmuphi(i,j)
               !
               amatrx(A11+3,B11+7) = KphiomgH(i,j)
               !
               amatrx(A11+3,B11+5) = KphiomgP(i,j)
               !
               amatrx(A11+3,B11+6) = KphiomgN(i,j)
               ...
               !
               amatrx(A11+4,B11+7) = KmuomgH(i,j)
               !
               amatrx(A11+7,B11+4) = KomgHmu(i,j)
               !
               ...
               !
               amatrx(A11+5,B11+7) = KomgPomgH(i,j)
               !
               amatrx(A11+6,B11+7) = KomgNomgH(i,j)
               !
               amatrx(A11+7,B11+5) = KomgHomgP(i,j)
               !
               amatrx(A11+7,B11+6) = KomgHomgN(i,j)
            enddo
         enddo
         !
      else
...
************************************************************************
      ...
      subroutine AssembleElement2(nDim,nNode,ndofel,
     +     Ru,Rmu,RomgP,RomgN,Rphi,RomgH,
     +     Kuu,Kumu,Kmuu,Kmumu,KomgPomgP,KomgNomgN,Kphiphi,KomgHomgH,
     +     KomgPphi,KomgNphi,KphiomgP,KphiomgN,
     +     KmuomgN,KmuomgP,KomgPmu,KomgNmu,
     ...
     +     rhs,amatrx)
      ...
      if(nDim.eq.3) then
         ...
         do i=1,nNode
            do j=1,nNode
               ...
               ! chemical potential
               !
               amatrx(A11+8,B11+8) = KomgHomgH(i,j)
               !
               amatrx(A11+4,B11+4) = Kmumu(i,j)
               !
               amatrx(A11+5,B11+5) = KomgPomgP(i,j)
               ...
               !
               !
               amatrx(A11+8,B11) = KomgHu(i,B12)
               amatrx(A11+8,B11+1) = KomgHu(i,B12+1)
               amatrx(A11+8,B11+2) = KomgHu(i,B12+2)
               !
               amatrx(A11+4,B11) = Kmuu(i,B12)
               amatrx(A11+4,B11+1) = Kmuu(i,B12+1)
               amatrx(A11+4,B11+2) = Kmuu(i,B12+2)
               ...
               ! echemical potential - electrical potential
               !
               amatrx(A11+8,B11+3) = KomgHphi(i,j)
               !
               amatrx(A11+5,B11+3) = KomgPphi(i,j)
               !
               amatrx(A11+6,B11+3) = KomgNphi(i,j)
               ...
               amatrx(A11+4,B11+3) = Kmuphi(i,j)
               !
               amatrx(A11+3,B11+8) = KphiomgH(i,j)
               !
               amatrx(A11+3,B11+5) = KphiomgP(i,j)
               !
               amatrx(A11+3,B11+6) = KphiomgN(i,j)
               ...
               amatrx(A11+4,B11+6) = KmuomgN(i,j)
               !
               amatrx(A11+4,B11+8) = KmuomgH(i,j)
               !
               amatrx(A11+8,B11+4) = KomgHmu(i,j)
               !
               amatrx(A11+5,B11+4) = KomgPmu(i,j)
               !
               amatrx(A11+6,B11+4) = KomgNmu(i,j)
               ...
               amatrx(A11+6,B11+5) = KomgNomgP(i,j)
               !
               amatrx(A11+5,B11+8) = KomgPomgH(i,j)
               !
               amatrx(A11+6,B11+8) = KomgNomgH(i,j)
               !
               amatrx(A11+8,B11+5) = KomgHomgP(i,j)
               !
               amatrx(A11+8,B11+6) = KomgHomgN(i,j)
            enddo
         enddo
         !
      else
      ...
      return
      end subroutine AssembleElement2            
...
!****************************************************************************
      subroutine mapShape3Da(nNode,dshxi,coords,dsh,detMapJ,stat,pnewdt)
      !
      ! Map derivatives of shape fns from xi-eta-zeta domain
      !  to x-y-z domain.  This subroutine works for both 8-node
      !  linear and 20-node quadratic 3D elements.
