...
*
     ...
     +     (lflags(1).eq.72).or.(lflags(1).eq.73)) then
      ...
      if(jtype.eq.1) then
         !
         ! This is an plane strain analysis for the anionic polymer
         !
         nDim = 2
         call UPE4a(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
     +        PROPS,NPROPS,coords,MCRD,NNODE,Uall,DUall,Vel,Accn,JTYPE,
     +        TIME,DTIME,KSTEP,KINC,JELEM,PARAMS,NDLOAD,JDLTYP,ADLMAG,
     +        PREDEF,NPREDF,LFLAGS,MLVARX,DDLMAG,MDLOAD,PNEWDT,JPROPS,
     +        NJPROP,PERIOD,
     +        nDim,nInt,nIntS)
      ...
      elseif(jtype.eq.2) then
         !
         ! This is a plane strain analysis for the cationic polymer
         !
         nDim = 2
         call UPE4b(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
     +        PROPS,NPROPS,coords,MCRD,NNODE,Uall,DUall,Vel,Accn,JTYPE,
     +        TIME,DTIME,KSTEP,KINC,JELEM,PARAMS,NDLOAD,JDLTYP,ADLMAG,
     +        PREDEF,NPREDF,LFLAGS,MLVARX,DDLMAG,MDLOAD,PNEWDT,JPROPS,
     +        NJPROP,PERIOD,
...
************************************************************************
      ...
      subroutine UPE4a(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
     +     PROPS,NPROPS,coords,MCRD,NNODE,Uall,DUall,Vel,Accn,JTYPE,
     +     TIME,DTIME,KSTEP,KINC,JELEM,PARAMS,NDLOAD,JDLTYP,ADLMAG,
     +     PREDEF,NPREDF,LFLAGS,MLVARX,DDLMAG,MDLOAD,PNEWDT,JPROPS,
     +     NJPROP,PERIOD,
...
!     +        + matmul(matmul(dshC,AmatCU),Gmat)
     ...
     +     rhs,amatrx)
      ...
      return
      end subroutine UPE4a

************************************************************************

      subroutine UPE4b(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
     +     PROPS,NPROPS,coords,MCRD,NNODE,Uall,DUall,Vel,Accn,JTYPE,
     +     TIME,DTIME,KSTEP,KINC,JELEM,PARAMS,NDLOAD,JDLTYP,ADLMAG,
     +     PREDEF,NPREDF,LFLAGS,MLVARX,DDLMAG,MDLOAD,PNEWDT,JPROPS,
     +     NJPROP,PERIOD,
...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 cRf_dissoc,Kb_NH3,cOH_eff
      real*8 rR_OH
      real*8 dcRf_dissdcOH_eff,dcOH_effdomgOH
      real*8 dcOH_effdphi,dcOH_effdzeta,dcOH_effdmu
...
c         deallocate(globalSdv)
     ...
     +        SpumuMod,SpmuumodFac)
         ...
         Kb_NH3 = 10.d0**(-6.0d0)*1.d3 
         cOH_eff = cOH_tau*zeta_tau/(one-zeta_tau)
         cRf_dissoc = cRf/(one+cOH_eff/Kb_NH3)
         
         rR_OH = 1/dtime*(cRf_dissoc-svars(5+jj))
         dcRf_dissdcOH_eff = -cRf_dissoc/(Kb_NH3+cOH_eff)
         dcOH_effdomgOH = dcOHdomgOH*zeta_tau/(one-zeta_tau)
         dcOH_effdphi = dcOHdphi*zeta_tau/(one-zeta_tau)
         dcOH_effdmu = dcOHdmu*zeta_tau/(one-zeta_tau)
         dcOH_effdzeta = cOH_tau*one/(one-zeta_tau)**two
...
!     +        )
     ...
     +           - 1/dtime*dcRf_dissdcOH_eff*dcOH_effdomgOH/detF
         !
     ...
     +        )
         ...
         TanFac = zP_mob*dcPdphi+zN_mob*dcNdphi
     +-zf*cRf_dissoc/(Kb_NH3+cOH_eff)*dcOHdphi*zeta_tau/(one-zeta_tau)
     +           +zOH_mob*dcOHdphi 
     ...
     +           - 1/dtime*dcRf_dissdcOH_eff*dcOH_effdphi/detF
         !
     ...
     +       -zf*cRf_dissoc/(Kb_NH3+cOH_eff)*dcOHdomgOH
     +                             *zeta_tau/(one-zeta_tau)
     ...
     +           - 1/dtime*dcRf_dissdcOH_eff*dcOH_effdmu/detF
     +           - 1/dtime*dcRf_dissdcOH_eff*dcOH_effdzeta*dzetadmu/detF
...
!     +        + matmul(matmul(dshC,AmatCU),Gmat)
     ...
     +        ( zero
     +        + mobOH/mobS*matmul(matmul(dshC,AmatCU),Gmat)
!     +        + matmul(matmul(dshC,AmatCU),Gmat)
     +        )
