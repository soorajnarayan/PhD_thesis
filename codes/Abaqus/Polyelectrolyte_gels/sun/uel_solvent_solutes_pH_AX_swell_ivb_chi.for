...
*
     ...
     +     (lflags(1).eq.72).or.(lflags(1).eq.73)) then
      ...
      if(jtype.eq.1) then
         ...
         !
         nDim = 2
c         call UPE4(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
...
*
     ...
     4 PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      real*8 omgOHOld(nNode),domgOH(nNode),omgOHNew(nNode)
      ...
      real*8 omgOH_tau,omgOH_t,domgOHdx(2,1),domgOHdt
      ...
      real*8 KmuomgOH(nNode,nNode),KomgOHmu(nNode,nNode)
      real*8 KomgNomgP(nNode,nNode),KomgPomgN(nNode,nNode)
      real*8 KomgNomgOH(nNode,nNode),KomgPomgOH(nNode,nNode)
      real*8 KomgOHomgN(nNode,nNode),KomgOHomgP(nNode,nNode)
      
      real*8 KphiomgOH(nNode,nNode),KomgOHphi(nNode,nNode)
      real*8 KomgOHu(nNode,nDim*nNode),KuomgOH(nDim*nNode,nNode)
      ...
      real*8 DiffOH,zOH_mob,K3_rate,omgOH0
      ...
      real*8 Nvec(1,nNode),ResFac,AmatUC(4,1),TanFac
      real*8 dcPdphi,dcNdphi,dmobPdphi,dmobNdphi
      real*8 SpumuMod(3,3),Spmuumod(3,3,3),SpmuumodFac(3,3),AmatCU(2,5)
      ...
      real*8 cOHdot,dcOHdomgOH,dcOHdmu,mobOH,dmobOHdmu,dmobOHdomgOH
      real*8 dcOHdphi,dmobOHdphi,cOH_tau,cOH_t,cOH0
      real*8 cRf_dissoc,Kb_DMAEA,cOH_eff
      
      real*8 e_tau(3),e_tau_mod,permitt
...
c         deallocate(globalSdv)
      ...
      K3_rate =  5.d3*DiffOH*cOH0/Rgas/theta0/10.d-3
      ...
      do i=1,nNode
         ...
         omgOHNew(i) = Uall(k)
         domgOH(i) = DUall(k,1)
         omgOHOld(i) = omgOHNew(i) - domgOH(i)
         k = k + 1
         muNew(i) = Uall(k)
         dMU(i) = DUall(k,1)
      ...
      !
      do i=1,nNode
         if(dabs(domgOH(i)).gt.1.d6) then
            pnewdt = 0.5
            write(*,*)'Problem in domgOH'
            return
         endif
      enddo
      !
     ...
     +     ((coordsC(2,1)-coordsC(2,3))**two))
      ...
      do intpt=1,nIntPt
       ...
       ! For an axisymmetric problem, find the ``r'' that
         ...
         domgOHdx = zero
         do k=1,nNode
            omgOH_tau = omgOH_tau + omgOHNew(k)*sh(k)
            omgOH_t   = omgOH_t + omgOHOld(k)*sh(k)
            do i=1,nDim
               domgOHdx(i,1) = domgOHdx(i,1) + omgOHNew(k)*dshC(k,i)
            enddo
         enddo
         domgOHdt = (omgOH_tau - omgOH_t)/dtime
         omgOH_tau = omgOH_tau-omgOH0
         omgOH_t   = omgOH_t-omgOH0
     ...
     +        F_tau,mu_tau,omgP_tau,omgN_tau,phi_tau,omgOH_tau,    
     +        zeta_t,theta0,
     +        T_tau,SpTanMod,
     +        zeta_tau,dZdt,DzetaDmu,DzetadotDmu,
     +        mobS,dmobSdmu,dmobSdJ,Omega,
     +        mobP,dmobPdomgP,mobN,dmobNdomgN,mobOH,dmobOHdomgOH,
     +        cP_t,cN_t,cOH_t,cRf,cRf_dissoc,
     +        cP_tau,cN_tau,dcPdomgP,dcNdomgN,cOH_tau,dcOHdomgOH,
     +        dcPdmu,dcNdmu,dmobPdmu,dmobNdmu,dcOHdmu,dmobOHdmu,
     +        dcPdphi,dcNdphi,dmobPdphi,dmobNdphi,dcOHdphi,dmobOHdphi,  
     +        SpumuMod,SpmuumodFac)
         ...
         Kb_DMAEA = 10.d0**(-4.6d0)*1.d3 
         cOH_eff = cOH_tau*zeta_tau/(one-zeta_tau)
         cRf_dissoc = cRf/(one+cOH_eff/Kb_DMAEA)
     ...
     +                                -half*(e_tau_mod)*Iden(i,j) )
         ...
         globalSdv(jelem,intPt,5) = cPdot
         globalSdv(jelem,intPt,6) = cNdot
         globalSdv(jelem,intPt,7) = cOH_tau
     ...
     +        - mobS/cS_tau*cOH_tau*matmul(dshC,domgOHdx)     
     +        )
     ...
     +                  matmul(dshC,domgNdX)
     +        - mobS*(cP_tau*cOH_tau/cS_tau**two)*
     +                  matmul(dshC,domgOHdX)
     +        )
         !RomgP = zero
     ...
     +        - mobS*(cP_tau*cN_tau/cS_tau**two)*
     +                  matmul(dshC,domgPdX)    
     +        - mobS*(cOH_tau*cN_tau/cS_tau**two)*
     +                  matmul(dshC,domgOHdX)    
     +        )
         !RomgN = zero
         ResFac = -cOHdot/detF
         !
         RomgOH  = RomgOH + detmapJC*w(intpt)*AR*
     +        (
     +        transpose(Nvec)*ResFac 
     +        - mobOH*matmul(dshC,domgOHdX)
     +        - mobS*(cOH_tau/cS_tau)**two*matmul(dshC,domgOHdX)
     +        - mobS*(cOH_tau/cS_tau)*
     +                matmul(dshC,dmudX)
     +        - mobS*(cP_tau*cOH_tau/cS_tau**two)*
     +                  matmul(dshC,domgPdX)    
     +        - mobS*(cN_tau*cOH_tau/cS_tau**two)*
     +                  matmul(dshC,domgNdX)   
     +        ) 
     ...
     +     +  zf*cRf_dissoc+ zOH_mob*(cOH_tau)
!     +    - zOH_mob*(1.d-8/cOH_tau)*(one/zeta_tau-one)**two
!     +      + zf*cRf_dissoc+ zOH_mob*(cOH_tau)- zOH_mob*(1.d-8/cOH_tau)
         !
...
!     +        )
     ...
     +        + mobS/cS_tau*dcOHdmu*matmul(matmul(dshC,domgOHdX),Nvec)
     +        )
     ...
     +        + mobS*(cOH_tau/cS_tau**two)*dcPdomgP*
     +                                matmul(matmul(dshC,domgOHdX),Nvec)
     +        )
     ...
     +                                 matmul(matmul(dshC,domgPdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau**two)*dcNdomgN*
     +                                matmul(matmul(dshC,domgOHdX),Nvec)
     +        )
         
         TanFac = 1/dtime*dcOHdomgOH/detF
         !
         KomgOHomgOH = KomgOHomgOH + detmapJC*w(intPt)*AR*
     +        (
     +        TanFac*matmul(transpose(Nvec),Nvec)
     +        + mobOH*matmul(dshC,transpose(dshC))
     +        + dmobOHdomgOH*matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau)**two*matmul(dshC,transpose(dshC))
     +        + mobS*(two*cN_tau/cS_tau**two)*dcOHdomgOH
     +                              *matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS/cS_tau*dcOHdomgOH*
     +                                 matmul(matmul(dshC,dmudX),Nvec)
     +        + mobS*(cP_tau/cS_tau**two)*dcOHdomgOH*
     +                                 matmul(matmul(dshC,domgPdX),Nvec)
     +        + mobS*(cN_tau/cS_tau**two)*dcOHdomgOH*
     +                                matmul(matmul(dshC,domgNdX),Nvec)
     +        )
     ...
     +-zf*cRf_dissoc/(Kb_DMAEA+cOH_eff)*dcOHdphi*zeta_tau/(one-zeta_tau)
     +           +zOH_mob*dcOHdphi 
!     +    +zOH_mob/cOH_tau**two*1.d-8*dcOHdphi*(one/zeta_tau-one)**two
     ...
     +                                 matmul(matmul(dshC,domgNdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau**two)*dcPdphi*
     +                                matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cP_tau/cS_tau**two)*dcOHdphi*
     +                                matmul(matmul(dshC,domgOHdX),Nvec)
     +        )
     ...
     +        + mobS*(cN_tau/cS_tau**two)*dcPdphi*
     +                                 matmul(matmul(dshC,domgPdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau**two)*dcNdphi*
     +                                matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cN_tau/cS_tau**two)*dcOHdphi*
     +                                matmul(matmul(dshC,domgOHdX),Nvec)
     +        )
         
         TanFac = 1/dtime*dcOHdphi/detF
         !
         KomgOHphi = KomgOHphi + detmapJC*w(intPt)*AR*
     +        (
     +        TanFac*matmul(transpose(Nvec),Nvec)
     +        + dmobOHdphi*matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(two*cOH_tau/cS_tau**two)*dcOHdphi
     +                           *matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS/cS_tau*dcOHdphi*
     +                                 matmul(matmul(dshC,dmudX),Nvec)
     +        + mobS*(cP_tau/cS_tau**two)*dcOHdphi*
     +                                 matmul(matmul(dshC,domgPdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau**two)*dcPdphi*
     ...
     +        )
         ...
         TanFac = zOH_mob*dcOHdomgOH 
     +       -zf*cRf_dissoc/(Kb_DMAEA+cOH_eff)*dcOHdomgOH
     +                             *zeta_tau/(one-zeta_tau)
!     +    +zOH_mob/cOH_tau**two*1.d-8*dcOHdomgOH*(one/zeta_tau-one)**two
         !
         KphiomgOH = KphiomgOH + detmapJC*w(intPt)*AR*
     +        (
     +        TanFac*matmul(transpose(Nvec),Nvec)
     +        )
     ...
     +        )
         ...
         KmuomgOH = KmuomgOH + detmapJC*w(intPt)*AR*
     +        ( zero
     +        + mobS/cS_tau*cOH_tau*matmul(dshC,transpose(dshC))
     +        + mobS/cS_tau*dcOHdomgOH*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        )
     ...
     +                            matmul(matmul(dshC,domgNdX),Nvec)
     +        - dmobSdmu*(cP_tau*cOH_tau/cS_tau**two)*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau**two)*dcPdmu*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cP_tau/cS_tau**two)*dcOHdmu*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        ) 
     ...
     +        + mobS*(cP_tau/cS_tau**two)*dcNdmu*
     +                            matmul(matmul(dshC,domgPdX),Nvec)
     +        - dmobSdmu*(cN_tau*cOH_tau/cS_tau**two)*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau**two)*dcNdmu*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cN_tau/cS_tau**two)*dcOHdmu*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        ) 
          
          TanFac = 1/dtime*dcOHdmu/detF
         KomgOHmu = KomgOHmu + detmapJC*w(intpt)*AR*
     +        ( zero
     +        + TanFac*matmul(transpose(Nvec),Nvec)
     +        + dmobOHdmu*matmul(matmul(dshC,domgOHdX),Nvec)
     +        - dmobSdmu*(cOH_tau/cS_tau)**two*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(two*cOH_tau/cS_tau**two)*dcOHdmu*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS/cS_tau*dcOHdmu*
     +                            matmul(matmul(dshC,dmudX),Nvec)   
     +        + mobS*(cOH_tau/cS_tau)*
     +              matmul(dshC,transpose(dshC))
     +        - dmobSdmu*(cOH_tau*cP_tau/cS_tau**two)*
     +                            matmul(matmul(dshC,domgPdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau**two)*dcPdmu*
     +                            matmul(matmul(dshC,domgPdX),Nvec)
     +        + mobS*(cP_tau/cS_tau**two)*dcOHdmu*
     +                            matmul(matmul(dshC,domgPdX),Nvec)
     +        - dmobSdmu*(cOH_tau*cN_tau/cS_tau**two)*
     +                            matmul(matmul(dshC,domgNdX),Nvec)
     +        + mobS*(cN_tau/cS_tau**two)*dcOHdmu*
     +                            matmul(matmul(dshC,domgNdX),Nvec)
     +        + mobS*(cOH_tau/cS_tau**two)*dcNdmu*
     ...
     +        )
         ...
         KomgPomgOH = KomgPomgOH + detmapJC*w(intPt)*AR*
     +        ( zero
     +        + mobS*(cP_tau/cS_tau**two)*dcOHdomgOH*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cP_tau*cOH_tau/cS_tau**two)*
     ...
     +        )
         ...
         KomgNomgOH = KomgNomgOH + detmapJC*w(intPt)*AR*
     +        ( zero
     +        + mobS*(cN_tau/cS_tau**two)*dcOHdomgOH*
     +                            matmul(matmul(dshC,domgOHdX),Nvec)
     +        + mobS*(cN_tau*cOH_tau/cS_tau**two)*
     ...
     +        )
         ...
         KomgOHomgP = KomgOHomgP + detmapJC*w(intPt)*AR*
     +        ( zero
     +        + mobS*(cOH_tau/cS_tau**two)*dcPdomgP*
     +                            matmul(matmul(dshC,domgPdX),Nvec)
     +        + mobS*(cOH_tau*cP_tau/cS_tau**two)*
     +                            matmul(dshC,transpose(dshC))    
     +        )
         
         KomgOHomgN = KomgOHomgN + detmapJC*w(intPt)*AR*
     +        ( zero
     +        + mobS*(cOH_tau/cS_tau**two)*dcNdomgN*
     +                            matmul(matmul(dshC,domgNdX),Nvec)
     +        + mobS*(cOH_tau*cN_tau/cS_tau**two)*
...
!     +           - zf*cRf_dissoc/(Kb_DMAEA+cOH_tau)*dcOHdmu
     +           +zOH_mob*dcOHdmu 
!     +    +zOH_mob/cOH_tau**two*1.d-8*dcOHdmu*(one/zeta_tau-one)**two
!     +   -zOH_mob/cOH_tau*1.d-8*two*(one/zeta_tau-one)
!     +                              *(-one/zeta_tau**two)*dzetadmu 
     ...
     +        ( zero
     +        + mobP/mobS*matmul(matmul(dshC,AmatCU),GmatAx)
!     +        + matmul(matmul(dshC,AmatCU),Gmat)
     +        )
         
         KomgNu = KomgNu - detMapJC*w(intpt)*AR*
     +        ( zero
     +        + mobN/mobS*matmul(matmul(dshC,AmatCU),GmatAx)
!     +        + matmul(matmul(dshC,AmatCU),Gmat)
     +        )
         
         KomgOHu = KomgOHu - detMapJC*w(intpt)*AR*
     +        ( zero
     +        + mobOH/mobS*matmul(matmul(dshC,AmatCU),GmatAx)
!     +        + matmul(matmul(dshC,AmatCU),Gmat)
     +        )
     ...
     +                 sh,ds)
                  ...
                  flux_net = -K3_rate * (flux - omgOH_tau)
                  RomgOH(1,1) = RomgOH(1,1) - w(ii)*ds*AR*sh(1)*flux_net
                  RomgOH(2,1) = RomgOH(2,1) - w(ii)*ds*AR*sh(2)*flux_net


                  ! Change on the tangent matrix
                  !
                  do k=1,nNode
                  KomgOHomgOH(1,k) = KomgOHomgOH(1,k)+w(ii)*ds*AR*sh(1)*
     +                            (K3_rate)*sh(k)
                  
                  KomgOHomgOH(2,k) = KomgOHomgOH(2,k)+w(ii)*ds*AR*sh(2)*
     +                            (K3_rate)*sh(k)
     ...
     +                 sh,ds)
                  ...
                  flux_net = -K3_rate * (flux - omgOH_tau)
                  RomgOH(2,1) = RomgOH(2,1) - w(ii)*ds*AR*sh(2)*flux_net
                  RomgOH(3,1) = RomgOH(3,1) - w(ii)*ds*AR*sh(3)*flux_net
     ...
     +                            (K3_rate)*sh(k)
                  ...
                  KomgOHomgOH(3,k) = KomgOHomgOH(3,k)+w(ii)*ds*AR*sh(3)*
     +                            (K3_rate)*sh(k)
       
                  enddo
               enddo ! end loop over integ points 
               
               elseif(face.eq.33) then
               !
               ! flux on face 3 of the element
               !
               xLocal(1) = -dsqrt(one/three)
     ...
     +                 sh,ds)
                  ...
                  flux_net = -K3_rate * (flux - omgOH_tau)
                  RomgOH(3,1) = RomgOH(3,1) - w(ii)*ds*AR*sh(3)*flux_net
                  RomgOH(4,1) = RomgOH(4,1) - w(ii)*ds*AR*sh(4)*flux_net
     ...
     +                            (K3_rate)*sh(k)
                  ...
                  KomgOHomgOH(4,k) = KomgOHomgOH(4,k)+w(ii)*ds*AR*sh(4)*
     +                            (K3_rate)*sh(k)
       
                  enddo
               enddo ! end loop over integ points
               
               elseif(face.eq.34) then
               !
               ! flux on face 4 of the element
               !
               xLocal(1) = -one
     ...
     +     Ru,Rmu,RomgP,RomgN,Rphi,RomgOH,
     +     Kuu,Kumu,Kmuu,Kmumu,KomgPomgP,KomgNomgN,Kphiphi,KomgOHomgOH,
     +     KomgPphi,KomgNphi,KphiomgP,KphiomgN,
     +     KmuomgN,KmuomgP,KomgPmu,KomgNmu,
     +     KomgPomgN,KomgNomgP,
     +     KomgOHomgP,KomgOHomgN,
     +     KomgPomgOH,KomgNomgOH,KmuomgOH,
     +     KomgPu,KomgNu,Kphimu,Kmuphi,KomgOHu,KomgOHphi,KphiomgOH,
     +     KomgOHmu, 
     +     rhs,amatrx)
      !
...
************************************************************************
     ...
     +     SpUmuMod,SpmuUModFac)
      ...
      real*8 omgP_tau,omgN_tau,phi_tau,omgOH_tau
      
      real*8 zeta_tau,dZdt,DzetaDmu,DzetadotDmu,mobS
      real*8 DmobSDmu,DzetaDJ,DmobSDzeta,DmobSDJ
      real*8 mobP,dmobPdomgP,mobN,dmobNdomgN,mobOH,dmobOHdomgOH
      real*8 cP_tau,cN_tau,dcPdomgP,dcNdomgN,cOH_tau,dcOHdomgOH
      real*8 cP_t,cN_t,cOH_t, cH_t
      real*8 dcPdmu,dcNdmu,dmobPdmu,dmobNdmu,dcOHdmu,dmobOHdmu
      real*8 dcPdphi,dcNdphi,dmobPdphi,dmobNdphi,dcOHdphi,dmobOHdphi
     ...
     +     half=1.d0/2.d0,Farad=96485.33d0)
      ...
      cH_t = 1.d-8/cOH_t*(one/zeta_t-one)**two
     ...
     +           *dexp((omgN_tau-zN_mob*Farad*phi_tau)/(Rgas*theta))
      ...
      cOH_tau = (one - zeta_tau)/(Omega*zeta_tau)*
     +         dexp((omgOH_tau-zOH_mob*Farad*phi_tau)/(Rgas*theta))
      ...
      dcOHdomgOH= cOH_tau/(Rgas*theta)
      dcOHdphi  = -cOH_tau/(Rgas*theta)*zOH_mob*Farad
...
************************************************************************
     ...
     +     rhs,amatrx)
      ...
      real*8 Rphi(nNode,1),RomgOH(nNode,1),KomgOHomgOH(nNode,nNode)
      ...
      real*8 KomgOHu(nNode,nDim*nNode),KomgOHphi(nNode,nNode)
      real*8 KphiomgOH(nNode,nNode),KomgOHmu(nNode,nNode)
      real*8 KomgPomgOH(nNode,nNode),KomgNomgOH(nNode,nNode)
      real*8 KmuomgOH(nNode,nNode)
      real*8 KomgOHomgP(nNode,nNode),KomgOHomgN(nNode,nNode)
      ...
      if(nDim.eq.2) then
         ...
         do i=1,nNode
            do j=1,nNode
               ...
               ! chemical potential
               !
               amatrx(A11+3,B11+3) = KomgOHomgOH(i,j)
               !
               ...
               !
               !
               amatrx(A11+3,B11) = KomgOHu(i,B12)
               amatrx(A11+3,B11+1) = KomgOHu(i,B12+1)
               !
               ...
               !
               amatrx(A11+3,B11+2) = KomgOHphi(i,j)
               !
               ...
               !
               amatrx(A11+2,B11+3) = KphiomgOH(i,j)
               !
               ...
               !
               amatrx(A11+4,B11+3) = KmuomgOH(i,j)
               !
               amatrx(A11+3,B11+4) = KomgOHmu(i,j)
               !
               ...
               !
               amatrx(A11+5,B11+3) = KomgPomgOH(i,j)
               !
               amatrx(A11+6,B11+3) = KomgNomgOH(i,j)
               !
               amatrx(A11+3,B11+5) = KomgOHomgP(i,j)
               !
               amatrx(A11+3,B11+6) = KomgOHomgN(i,j)
            enddo
         enddo
         !
      else
...
!************************************************************************
     ...
     +    .or.(face.eq.14).or.(face.eq.22).or.(face.eq.24)
     +    .or.(face.eq.32).or.(face.eq.34) ) then
         ds = dsqrt(dXdEta*dXdEta + dYdEta*dYdEta)
      elseif( (face.eq.1).or.(face.eq.3).or.(face.eq.11)
     +    .or.(face.eq.13).or.(face.eq.21).or.(face.eq.23)
     +    .or.(face.eq.31).or.(face.eq.33) ) then
...
!*************************************************************************
     ...
     +     eighth=1.d0/8.d0)
      ...
      if(stat.eq.0) then
		 ...
		 write(*,*) 'Too big a jump, so cutting back.'
         pnewdt = 0.32
      endif
